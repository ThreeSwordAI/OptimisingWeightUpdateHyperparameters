...
def ray_tune_run_pbt(name, local_dir="./ray_results"):
    """Execute Population-Based Training (PBT) runs using Ray Tune."""
    def clip_momentum(config):
        config['momentum'] = min(max(config['momentum'], 0), 1 - 1e-6)
        return config
...
def ray_tune_run_asha(name, local_dir="./ray_results"):
    """Execute Asynchronous HyperBand (ASHA) runs using Ray Tune."""
...
def ray_tune_trainable(config, checkpoint_dir=None):
    ...
    os.chdir(pathlib.Path(__file__).parent.resolve())  # Use relative path
